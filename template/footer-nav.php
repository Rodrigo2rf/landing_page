<!-- Rodape -->
<div class="container-fluid container-rodape pt-5 pb-2">

   <a href="javascript:">
      <div class="ir-para-topo">
         <img src="assets/img/icone_marcacao/arrow-up.svg"> 
      </div>
   </a>

   <div class="container">
      <div class="row">
         <div class="col-md-3 info-endereco">
            <p><strong>Endereço</strong></p>
            <p>Av. Miguel Castro, 1355, Dix-Sept Rosado, Natal/RN, CEP: 59062-000</p>
            <p><strong>Fone: 55 (84) 4009 5501<br>sec.depecom@liga.org.br</strong></p>
         </div>
         <div class="col d-none d-md-block offset-md-1">
            <p><strong><a href="<?=url_index?>">Home</a></strong></p>
         </div>
         <div class="col d-none d-md-block">
            <p><strong><a href="<?=url_cursos?>">Cursos</a></strong></p>
            <ul>
               <li><a href="<?=url_redirect_presencial?>">Cursos Presencias</a></li>
               <li><a href="<?=url_redirect_semipresencial?>">Cursos Semipresencias</a></li>
               <li><a href="<?=url_redirect_ead?>">Cursos Online/EAD</a></li>
            </ul>
         </div>
         <div class="col d-none d-md-block">
            <p><strong><a href="<?=url_calendario?>">Calendário</a></strong></p>
         </div>
         <div class="col d-none d-md-block">
            <p><strong><a href="<?=url_contato?>">Contato</a></strong></p>
         </div>
         <div class="d-none d-lg-block">
            <ul class="list-inline icons-redes-sociais text-right">
               <li class="list-inline-item">
                  <a href="#"><img src="assets/img/icones_redes_sociais/facebook.svg"></a>
               </li>
               <li class="list-inline-item">
                  <a href="#"><img src="assets/img/icones_redes_sociais/instagram.svg"></a>
               </li>
               <li class="list-inline-item">
                  <a href="#"><img src="assets/img/icones_redes_sociais/twitter.svg"></a>
               </li>
            </ul>
         </div>
      </div>
      <div class="row justify-content-end copyright">
         <div class="col-lg-4 col-md-12 text-center">
            <p>Copyright | Liga Contra o Cancer</p>
         </div>
         <div class="col-lg-4 col-md-12">
            <p>Desenvolvido por Rodrigo Reinaldo e Lúcio Silva</p>
         </div>
      </div>
   </div>
   
</div>
<!-- -->
